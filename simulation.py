...
RULESET_NAME = "MyRules30k_300.in"
MESSAGE_COST = 0
TCAM_SIZE = 300
MAX_PRIORITY = 65535
        ...
        if empty_pos != 0:
            ...
            if entry_below <= TCAM_SIZE and entry_above > 0:
                ...
                if len(down_path) < len(up_path):
                    rusen_update(self.tcam, self.result, rule, down_path)
                else:
                    rusen_update(self.tcam, self.result, rule, up_path)
...
if __name__=="__main__":
    ...
    for i in range(0, TCAM_SIZE):
        my_algorithm.insert(rules[i])
        clear_buffer()
    
    for i in range(TCAM_SIZE, 2 * TCAM_SIZE):
